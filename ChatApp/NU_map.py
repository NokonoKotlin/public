...
if __name__ == '__main__':
    ...
    for i in range(div_st+1):
        for j in range(div_su+1):
            ...
            VALUES ( %s , %s , %s , %s , %s , %s)"""
            , [str(Longitude) , str(Latitude) , str(radius) , ChatRoomName , ChatRoomName ,""])
            
            Cursor.execute("CREATE TABLE {}(timestamp double , message varchar(255) , usertype INT, IPAddress varchar(63) , date varchar(63) , MessageGlobalID INT, Likes INT default 0)".format(ChatRoomName))  
